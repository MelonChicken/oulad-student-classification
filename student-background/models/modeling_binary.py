# -*- coding: utf-8 -*-
"""
modeling_binary.py  –  이진 분류 버전
0 = Distinction + Pass  (success)
1 = Fail + Withdrawn    (at-risk)
ver1 / ver2  ×  week5 / week7 / week10  ×  4 모델
"""
...
def to_binary(label_series):
    # 0,1 → 0 (success) / 2,3 → 1 (at-risk)
    return (label_series >= 2).astype(int)

records = []

for ver in ["ver1", "ver2"]:
    for week in ["week5", "week7", "week10"]:
        path = os.path.join(DATA_DIR, f"{ver}_{week}.csv")
        ...
        print(f"\n{'='*55}")
        print(f"  {ver.upper()}  |  {week}  |  at-risk={y.sum()}  total={len(y)}")
        print(f"{'='*55}")
        ...
        for name, clf in MODELS.items():
            ...
            records.append({
                "version": ver, "week": week, "model": name,
...
out = os.path.join(RESULT_DIR, "results_binary.csv")
