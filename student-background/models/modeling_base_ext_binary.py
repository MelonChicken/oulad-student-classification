# -*- coding: utf-8 -*-
"""
modeling_base_ext_binary.py  –  base_ext 이진 분류
...
for week in ["week5", "week7", "week10"]:
    path = os.path.join(DATA_DIR, f"base_ext_{week}.csv")
    ...
    print(f"\n{'='*55}")
    print(f"  base_ext  |  {week}  |  at-risk={y.sum()}  total={len(y)}")
    print(f"{'='*55}")
    ...
    for name, clf in MODELS.items():
        ...
        records.append({
            "version": "base_ext", "week": week, "model": name,
...
out = os.path.join(RESULT_DIR, "results_base_ext_binary.csv")
